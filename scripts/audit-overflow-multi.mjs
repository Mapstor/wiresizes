// Run overflow check across all 53 pages at 5 viewport widths
...
for (const w of widths) {
  ...
  console.log(`\n=== ${w}px: ${overflowing.length}/${urls.length} pages overflow ===`);
  overflowing.sort((a, b) => b.overflow - a.overflow);
  for (const r of overflowing) {
    const c = r.culprits?.[0];
    console.log(`  ${(r.url.replace('https://wiresizes.com','') || '/').padEnd(45)} +${r.overflow}px ${c ? `<${c.tag} "${c.cls.slice(0,40)}" w=${c.width}>` : ''}`);
    if (c?.text) console.log(`    "${c.text}"`);
  }
}

await browser.close();
fs.writeFileSync('/tmp/wsaudit/overflow-multi.json', JSON.stringify(results, null, 2));
console.log(`\nSaved to /tmp/wsaudit/overflow-multi.json`);
